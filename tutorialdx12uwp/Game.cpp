...
void Game::Update(DX::StepTimer const& timer)
{
    ...
    auto elementSize= CalcConstantBufferByteSize(sizeof(vConstants));
    UINT count = 0;
    for (auto obj : objects) {
        ...
        XMStoreFloat4x4(&m_vConstants[m_backBufferIndex].GNormalTransform, XMMatrixTranspose(normaltransform));
        XMStoreFloat4x4(&m_vConstants[m_backBufferIndex].GTransform, XMMatrixTranspose(transform));
        
        memcpy(&data[count*elementSize], reinterpret_cast<const void*>(&m_vConstants[m_backBufferIndex]), sizeof(vConstants)); //Copia de la transformación
        ++count;
    }
...
void Game::Render()
{
    ...
    cHandle.Offset(m_backBufferIndex * objects.size(), m_cDescriptorSize);
...
void Game::Clear()
{
    ...
    */
    ...
    srvHandle.Offset(objects.size()*c_swapBufferCount, m_cDescriptorSize);
...
void Game::CreateMainInputFlowResources(const Mesh& mesh) {
    ...
    */
    ...
    cHeapDescriptor.NumDescriptors = (objects.size()*c_swapBufferCount)+1;
